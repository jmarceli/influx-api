...
import type { QueryParams, WriteParams, InfluxResponse, UrlParams } from './types';
...
export const query = async (params: QueryParams): Promise<InfluxResponse> => {
  return post(queryParams({ ...params, url: `${params.url}/query` }));
};

export const write = async (params: WriteParams): Promise<InfluxResponse> => {
  return post(writeParams({ ...params, url: `${params.url}/write` }));
};

// Converts connection params to URL
export const buildUrl = ({ host, ssl, port }: UrlParams): string => {
